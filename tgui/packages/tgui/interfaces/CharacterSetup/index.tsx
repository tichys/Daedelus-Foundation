...
const PriorityButtons = ({ job, prefs, act }) => (
  <Box style={{ display: 'flex', gap: '2px' }}>
    <TermButton
      selected={!prefs}
      onClick={() => act('set_job_priority', { job, level: null })}
    >
      OFF
    </TermButton>
    <TermButton
      color="red"
      selected={prefs === 1}
      onClick={() => act('set_job_priority', { job, level: 1 })}
    >
      LOW
    </TermButton>
    <TermButton
      color="yellow"
      selected={prefs === 2}
      onClick={() => act('set_job_priority', { job, level: 2 })}
    >
      MED
    </TermButton>
    <TermButton
      color="green"
      selected={prefs === 3}
      onClick={() => act('set_job_priority', { job, level: 3 })}
    >
      HIGH
    </TermButton>
  </Box>
  ...
  <TermBox>
    ...
    <TermRow>
      ...
      <TermValue bold color={C.amber}>
        {data.name_to_use || 'UNASSIGNED'}
      </TermValue>
    </TermRow>
    <TermRow>
      <TermLabel>FACTION</TermLabel>
      <TermValue>{data.faction || '—'}</TermValue>
    </TermRow>
    <TermRow>
      <TermLabel>CLASS</TermLabel>
      <TermValue>{data.class || '—'}</TermValue>
    </TermRow>
    <TermRow>
      <TermLabel>STATUS</TermLabel>
      <TermValue color={data.faction_class_locked ? C.green : C.amber}>
        {data.faction_class_locked ? 'LOCKED' : 'UNLOCKED'}
      </TermValue>
    </TermRow>
    <TermRow>
      <TermLabel>RESET TOKENS</TermLabel>
      <TermValue>{data.faction_class_reset_tokens || 0}</TermValue>
    </TermRow>
  </TermBox>
);
  ...
  return (
    <TermBox>
      ...
      <TermBox style={{ marginBottom: '8px' }}>
        <TermRow>
          <TermLabel>FACTION</TermLabel>
          <TermValue>{selectedFaction || '—'}</TermValue>
        </TermRow>
        <Box
          style={term({
            color: C.textDim,
      ...
      <TermBox>
        <TermRow>
          <TermLabel>CLASS</TermLabel>
          <TermValue>{selectedClass || '—'}</TermValue>
        </TermRow>
        <Box
          style={term({
            color: C.textDim,
        ...
        >
          {(selectedClass && classLore[selectedClass]) ||
            'No intelligence available.'}
        </Box>
      </TermBox>
      ...
      {locked ? (
        <Box>
          <TermRow>
            <TermValue color={C.green}>
              FACTION/CLASS COMMITTED AND LOCKED
            </TermValue>
          </TermRow>
          {tokens > 0 ? (
            confirmReset ? (
              <Box>
                ...
                <Box style={{ display: 'flex', gap: '4px' }}>
                  ...
                  <TermButton onClick={() => setConfirmReset(false)}>
                    CANCEL
                  </TermButton>
                </Box>
              </Box>
            ) : (
              <TermButton color="red" onClick={() => setConfirmReset(true)}>
                USE RESET TOKEN ({tokens} LEFT)
              </TermButton>
            )
          ) : (
            <Box style={term({ color: C.textDim, fontSize: '11px' })}>
              NO RESET TOKENS AVAILABLE
            </Box>
          )}
          {canAdmin && (
            <Box style={{ marginTop: '8px' }}>
              {confirmAdmin ? (
                <Box style={{ display: 'flex', gap: '4px' }}>
                  ...
                  <TermButton onClick={() => setConfirmAdmin(false)}>
                    CANCEL
                  </TermButton>
                </Box>
              ) : (
                <TermButton color="red" onClick={() => setConfirmAdmin(true)}>
                  ADMIN OVERRIDE
                </TermButton>
              )}
            </Box>
        ...
        <TermButton color="green" onClick={() => act('commit_faction_class')}>
          COMMIT AND LOCK
        </TermButton>
      )}
    </TermBox>
...
const JobsPage = () => {
  ...
  if (!data.faction || !data.class) {
    return (
      <TermBox>
        <NoticeBox>SELECT FACTION AND CLASS TO VIEW ASSIGNMENTS</NoticeBox>
      </TermBox>
    );
  }
  if (available.length === 0) {
    return (
      <TermBox>
        <NoticeBox>NO ASSIGNMENTS AVAILABLE</NoticeBox>
      </TermBox>
    );
  }
  ...
  return (
    <TermBox>
      <TermHeader>
        ASSIGNMENT PREFERENCES — {data.faction?.toUpperCase()} /{' '}
...
const CustomizationPage = () => {
  ...
  return (
    <TermBox>
      ...
      <Box style={{ marginBottom: '14px' }}>
        ...
        <Box style={{ marginTop: '4px' }}>
          <Dropdown
            ...
            displayText={
              speciesNames[currentSpeciesId] || currentSpeciesId || 'SELECT...'
            }
            onSelected={(value) =>
      ...
      <TermButton onClick={() => act('open_preferences')}>
...
const QuirksPage = () => {
  ...
  if (!list.length) {
    return (
      <TermBox>
        <NoticeBox>NO PSYCHOLOGICAL DATA ON FILE</NoticeBox>
      </TermBox>
    );
  }
...
const LanguagesPage = () => {
  ...
  if (!paths.length) {
    return (
      <TermBox>
        <NoticeBox>NO LANGUAGE DATA</NoticeBox>
      </TermBox>
    );
  }
...
const LoadoutPage = () => {
  ...
  return (
    <TermBox>
      ...
      <TermButton onClick={() => act('open_loadout')}>
  ...
  return (
    <TermBox>
      ...
      {entries.length > 0 ? (
        ...
        >
          NO MODIFICATIONS ON FILE
        </Box>
      )}
      <TermButton onClick={() => act('appearance_mods_act', { add: 1 })}>
        ADD MODIFICATION
      </TermButton>
    </TermBox>
  );
...
const AntagonistsPage = () => {
  ...
  if (entries.length === 0) {
    return (
      <TermBox>
        <NoticeBox>NO CLASSIFIED ROLES</NoticeBox>
      </TermBox>
    );
  }
  ...
  return (
    <TermBox>
      ...
      <Box style={{ display: 'flex', gap: '4px', marginBottom: '10px' }}>
        ...
        <TermButton onClick={() => act('antag_select_all_available')}>
          AVAILABLE
        </TermButton>
        <TermButton color="red" onClick={() => act('antag_deselect_all')}>
          NONE
        </TermButton>
      </Box>
      <Box style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
...
const FinalizePage = ({ data, act }) => (
  <TermBox>
    ...
    >
      WARNING: This action will persist all configuration data to the Foundation
      database and synchronize your personnel record. This cannot be undone.
    </Box>
    <OverviewPage data={data} />
    <TermDivider />
    <TermButton color="green" onClick={() => act('finalize')}>
      COMMIT TO DATABASE
    </TermButton>
  </TermBox>
);
      ...
      <Window.Content scrollable>
        ...
        >
          ...
          >
            ...
            >
              TERMINAL v4.7.2 | CLEARANCE LEVEL 2 | CLASSIFIED | ENCRYPTION:
              AES-512
            </Box>
          </Box>
          ...
          <Box style={{ display: 'flex' }}>
            ...
            >
              ...
              <TermButton fluid onClick={() => act('rotate')}>
                ROTATE
              </TermButton>
          ...
          >
            ...
            >
              SCP FOUNDATION | SECURE CONTAIN PROTECT | ALL ACTIVITY LOGGED |
