...
import { Box, Button, Collapsible, NoticeBox } from '../components';
...
export const PersistentProgressionClasses = (props) => {
  ...
  return (
    <Window title="SCP FOUNDATION — CLASS REGISTRY" width={900} height={700} theme="scp_terminal">
        ...
        <Box style={{
            ...
            {(!classes || classes.length === 0) ? (
              ...
              classes.map((classData) => (
                <Box key={classData.id} style={{ marginBottom: '12px' }}>
                  ...
                  <Box style={term({ color: C.textDim, fontSize: '11px', fontStyle: 'italic', marginBottom: '8px', paddingLeft: '8px' })}>
                    {classData.description}
                  </Box>
                  <TermRow>
                    <TermLabel>EXP MULTIPLIER</TermLabel>
                    <TermValue color={C.amber}>{classData.exp_multiplier}x</TermValue>
                  </TermRow>
                  <TermRow>
                    <TermLabel>MAX RANK</TermLabel>
                    <TermValue color={C.textBright}>{classData.max_rank}</TermValue>
                  </TermRow>
                  <TermRow>
                    <TermLabel>COMPATIBLE FACTIONS</TermLabel>
                    <TermValue color={C.textBright}>{classData.compatible_factions.join(', ')}</TermValue>
                  </TermRow>

                  <TermDivider />

                  <TermHeader>RANK PROGRESSION — {classData.name.toUpperCase()}</TermHeader>
                  {classData.ranks && classData.ranks.length > 0 ? (
                    classData.ranks.map((rank, index) => (
                      <Box key={index} style={{
                        ...
                        <TermValue bold color={rank.color || C.amber}>{rank.name}</TermValue>
                        <Box style={term({ color: C.textDim, fontSize: '10px', marginTop: '2px' })}>
                          {rank.requirement.toLocaleString()} XP REQUIRED
                        </Box>
                      </Box>
                    ))
                  ) : (
                    <Box style={term({ color: C.textDim, fontStyle: 'italic' })}>NO RANKS DEFINED</Box>
                  )}
          ...
          <Box style={{
            ...
            <Box style={term({ color: C.textDim, fontSize: '9px', letterSpacing: '0.1em' })}>
              SCP FOUNDATION | CLASS REGISTRY | ALL DATA CLASSIFIED | UNAUTHORIZED ACCESS IS A CLASS-B INFRACTION
            </Box>
          </Box>
        </Box>
      </Window.Content>
