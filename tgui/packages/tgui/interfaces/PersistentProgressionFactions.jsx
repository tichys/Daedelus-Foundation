...
import { Box, Button, Collapsible, Dropdown, NoticeBox } from '../components';
...
export const PersistentProgressionFactions = (props) => {
  ...
  return (
    <Window title="SCP FOUNDATION — FACTION REGISTRY" width={900} height={700} theme="scp_terminal">
        ...
        <Box style={{
              ...
              factions.map((factionData) => (
                <Box key={factionData.id} style={{ marginBottom: '12px' }}>
                  ...
                  <TermRow>
                    ...
                    <TermValue color={C.amber}>{factionData.exp_multiplier}x</TermValue>
                  </TermRow>

                  <TermDivider />

                  <TermHeader>ASSIGNED CLASSES — {factionData.name.toUpperCase()}</TermHeader>
                  {factionData.available_classes && factionData.available_classes.length > 0 ? (
                    factionData.available_classes.map((classData, index) => (
                      <Box key={index} style={{
                        ...
                        <TermValue bold color={C.amber}>{classData.name}</TermValue>
                    ...
                    <Box style={term({ color: C.textDim, fontStyle: 'italic' })}>NO CLASSES ASSIGNED</Box>
                  )}
          ...
          <Box style={{
            ...
            <Box style={term({ color: C.textDim, fontSize: '9px', letterSpacing: '0.1em' })}>
              SCP FOUNDATION | FACTION INTELLIGENCE | ALL DATA CLASSIFIED | UNAUTHORIZED ACCESS IS A CLASS-B INFRACTION
            </Box>
          </Box>
        </Box>
      </Window.Content>
