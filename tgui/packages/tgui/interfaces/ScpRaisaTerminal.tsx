    ...
    <Window theme="scp_terminal" width={700} height={650}>
      <Window.Content scrollable>
        ...
        <Section title="FILE INTELLIGENCE REPORT">
          <Box style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <Box style={{ display: 'flex', gap: '6px' }}>
              <Input placeholder="Target name..." value={raisaTarget} onInput={(_e, value: string) => setRaisaTarget(value)} style={{ width: '150px' }} />
              <Input placeholder="Target job..." value={raisaTargetJob} onInput={(_e, value: string) => setRaisaTargetJob(value)} style={{ width: '120px' }} />
            </Box>
            <Box style={{ display: 'flex', gap: '6px' }}>
              <Input placeholder="Report type..." value={raisaType} onInput={(_e, value: string) => setRaisaType(value)} style={{ width: '120px' }} />
              <Input placeholder="Classification..." value={raisaClass} onInput={(_e, value: string) => setRaisaClass(value)} style={{ width: '120px' }} />
            </Box>
        ...
        <Section title="SURVEILLANCE SUBJECTS">
          {subjects.map((s) => (
            <Box key={s.name} style={{ padding: '8px', marginBottom: '4px', borderLeft: `2px solid ${THREAT_COLOR(s.threat)}`, background: '#111114' }}>
              <Box style={{ display: 'flex', justifyContent: 'space-between' }}>
                <Box>
                  <Box style={{ color: THREAT_COLOR(s.threat), fontWeight: 'bold', fontFamily: 'monospace', fontSize: '12px' }}>
                  ...
                  {s.flag_reason && <Box style={{ fontSize: '10px', color: '#cc2222' }}>Reason: {s.flag_reason}</Box>}
                </Box>
                <Box style={{ display: 'flex', gap: '4px' }}>
                  <Button
                    onClick={() => act('observe_person', { target: s.name })}
                    style={{ fontFamily: 'monospace', fontSize: '9px', background: 'rgba(68,136,255,0.2)', border: '1px solid #4488ff', color: '#4488ff', padding: '2px 6px' }}
                  >
                    OBSERVE
                  </Button>
                  {!s.flagged && (
                    <Button
                      ...
                      style={{ fontFamily: 'monospace', fontSize: '9px', background: 'rgba(139,0,0,0.3)', border: '1px solid #8b0000', color: '#cc2222', padding: '2px 6px' }}
                    >
                      FLAG
                    </Button>
                  )}
        ...
        {breaches.length > 0 && (
          <Section title="INFORMATION BREACHES">
            {breaches.map((b) => (
              <Box key={b.breach_id} style={{ padding: '8px', marginBottom: '6px', borderLeft: `2px solid ${b.contained ? '#44ff44' : '#cc2222'}`, background: '#111114' }}>
                <Box style={{ color: b.contained ? '#44ff44' : '#cc2222', fontWeight: 'bold', fontFamily: 'monospace', fontSize: '12px' }}>
                ...
                {b.contained && <Box style={{ fontSize: '10px', color: '#44ff44', fontFamily: 'monospace', marginTop: '4px' }}>CONTAINED</Box>}
              </Box>
            ))}
          </Section>
        )}
