...
/obj/effect/corrosion_zone/New()
	. = ..()

/obj/effect/corrosion_zone/process()
	damage_tick++
	if(damage_tick >= damage_interval)
		damage_tick = 0
		apply_zone_damage()

/obj/effect/corrosion_zone/proc/apply_zone_damage()
...
/datum/scp106_containment_system/proc/force_return_to_containment()
	...
	for(var/obj/machinery/scp_femur_breaker/fb in world)
		containment_turf = get_turf(fb)
		break

	if(!containment_turf)
		for(var/area/scp/hcz/keter_containment/KA in world)
			for(var/turf/T in KA)
				if(istype(T, /turf/open))
					containment_turf = T
					break
			if(containment_turf)
