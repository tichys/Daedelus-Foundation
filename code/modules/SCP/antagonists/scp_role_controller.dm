...
/datum/scp_role_controller/proc/offer_scp_role(mob/dead/observer/ghost, scp_type)
	...
	var/datum/scp/scp_datum_ref = get_scp_datum_for_type(scp_type)
	if(scp_datum_ref)
		if(length(GLOB.clients) < scp_datum_ref.min_playercount)
			return FALSE
		if(world.time < scp_datum_ref.min_time)
			return FALSE
...
/datum/scp_role_controller/proc/assign_scp_role(mob/dead/observer/ghost, scp_type)
	...
	var/mob/living/carbon/human/scp_mob = new mob_type(pick(GLOB.scp_spawn_turfs))
	...
	if(antag_type)
		var/datum/antagonist/scp/antag = scp_mind.add_antag_datum(antag_type)
		if(antag)
			antag.forge_scp_objectives()

	SSticker.minds += scp_mind
	active_scp_minds[scp_mob.ckey] = scp_mind
...
/datum/scp_role_controller/proc/get_available_scp_types()
	...
	for(var/scp_type in list(SCP_ROLE_173, SCP_ROLE_096, SCP_ROLE_049, SCP_ROLE_106, SCP_ROLE_035, SCP_ROLE_457, SCP_ROLE_939, SCP_ROLE_079, SCP_ROLE_682, SCP_ROLE_008))
		...
		if(role_cooldowns[scp_type] && world.time < role_cooldowns[scp_type])
			continue
...
/datum/scp_role_controller/proc/get_mob_type(scp_type)
	switch(scp_type)
		if(SCP_ROLE_173)
			return /mob/living/carbon/human/scp173
		if(SCP_ROLE_096)
			return /mob/living/carbon/human/scp096
		if(SCP_ROLE_008)
			return /mob/living/carbon/human/scp008_carrier
		if(SCP_ROLE_035)
			return /mob/living/carbon/human
		if(SCP_ROLE_049)
			return /mob/living/carbon/human/scp049
		if(SCP_ROLE_079)
			return /mob/living/simple_animal/silicon/scp079
		if(SCP_ROLE_106)
			return /mob/living/carbon/human/scp106
		if(SCP_ROLE_457)
			return /mob/living/carbon/human/scp457
		if(SCP_ROLE_939)
			return /mob/living/carbon/human/scp939
		if(SCP_ROLE_682)
			return /mob/living/carbon/human/scp682
...
/datum/scp_role_controller/proc/get_scp_datum_for_type(scp_type)
	var/mob_type = get_mob_type(scp_type)
	if(!mob_type)
		return null
...
GLOBAL_LIST_INIT(scp_spawn_turfs, list())

/proc/find_scp_spawn_turfs()
	GLOB.scp_spawn_turfs.Cut()
	for(var/turf/T in world)
		if(istype(T.loc, /area/scp/lcz))
