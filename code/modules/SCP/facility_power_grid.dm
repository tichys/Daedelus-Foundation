	...
	switch(severity)
		if(1)
			...
			for(var/mob/living/carbon/human/H in zone.get_living_mobs())
				if(prob(40))
					to_chat(H, span_warning("The lights flicker momentarily..."))
					if(H.sanity)
		...
		if(2)
			...
			for(var/mob/living/carbon/human/H in zone.get_living_mobs())
				to_chat(H, span_danger("Emergency lighting activates as power fails across the zone!"))
				if(H.sanity)
		...
		if(3)
			...
			for(var/mob/living/carbon/human/H in zone.get_living_mobs())
				to_chat(H, span_userdanger("Total power failure! All systems offline!"))
				if(H.sanity)
...
/datum/controller/subsystem/facility_power/proc/restore_zone(zone_tag)
	...
	update_grid_integrity()
	for(var/mob/living/carbon/human/H in zone.get_living_mobs())
		to_chat(H, span_notice("Power has been restored to the zone. Systems coming back online."))
		if(H.sanity)
...
/obj/machinery/computer/facility_power_console/ui_act(action, params, datum/tgui/ui, datum/ui_state/state)
	...
	if(!id_card || !(ACCESS_ENGINEERING in id_card.access))
		to_chat(H, span_warning("Requires Engineering access."))
		return
	switch(action)
		...
		if("view_zone_details")
			...
			to_chat(H, span_notice("[details.Join("<br>")]"))
			. = TRUE
