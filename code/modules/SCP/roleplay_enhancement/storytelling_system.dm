...
/datum/storytelling_manager/proc/announce_arc_creation(datum/story_arc/arc)
	...
	switch(arc.arc_type)
		if(STORY_ARC_BREACH, STORY_ARC_CASCADE)
			priority_announce("NARRATIVE TRACKER: Containment breach arc initiated for [arc.scp_id]. All relevant personnel take note.", sound_type = ANNOUNCER_ALERT)
		if(STORY_ARC_RIOT)
			priority_announce("NARRATIVE TRACKER: D-Class unrest arc initiated. Security personnel be advised.", sound_type = ANNOUNCER_ALERT)
		if(STORY_ARC_CI_RAID)
			priority_announce("NARRATIVE TRACKER: Hostile incursion arc initiated. All hands to battle stations.", sound_type = ANNOUNCER_ALERT)
		if(STORY_ARC_OUTBREAK)
			priority_announce("NARRATIVE TRACKER: Pathogen outbreak arc initiated. Medical personnel respond.", sound_type = ANNOUNCER_ALERT)
		if(STORY_ARC_CORRUPTION)
			priority_announce("NARRATIVE TRACKER: Anomalous corruption detected. Research and security personnel investigate.", sound_type = ANNOUNCER_ALERT)
		if(STORY_ARC_TAKEOVER)
			priority_announce("NARRATIVE TRACKER: AI system anomaly detected. Engineering and command personnel respond.", sound_type = ANNOUNCER_ALERT)
