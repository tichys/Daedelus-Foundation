...
/datum/foundation_politics_manager/proc/enact_policy(dept_id, policy_type)
	...
	priority_announce("POLICY UPDATE: [dept.department_name] has enacted [replacetext(policy_type, "_", " ")]. All personnel take note.", sound_type = ANNOUNCER_DEFAULT)
	...
	switch(purchase_data["effect"])
		...
		if("research_grant")
			if(SSscp_research && SSscp_research.manager)
				SSscp_research.manager.total_research_points += 2000
			for(var/ckey in dept.department_members)
				var/mob/living/carbon/human/H = dept.get_mob_by_ckey(ckey)
				if(H && !QDELETED(H) && H.stat != DEAD)
...
/datum/foundation_politics_manager/proc/check_tension_thresholds()
	...
	if(political_tensions >= TENSION_CRITICAL)
		if(prob(10))
			priority_announce("WARNING: Political tensions in the facility have reached critical levels. Department heads, resolve your conflicts immediately.", sound_type = ANNOUNCER_ALERT)
		if(SSsecurity_level && SSsecurity_level.current_level < SEC_LEVEL_RED && prob(5))
			set_security_level(SEC_LEVEL_RED)
			priority_announce("ELEVATED ALERT: Facility political tensions have triggered automatic security escalation.", sound_type = ANNOUNCER_ALERT)
		...
		if(prob(5))
			...
			priority_announce("ADVISORY: [alert_msg]", sound_type = ANNOUNCER_DEFAULT)
	...
	if(political_tensions <= TENSION_LOW)
		if(prob(5))
			priority_announce("STATUS: Facility political tensions are low. All departments operating smoothly.", sound_type = ANNOUNCER_DEFAULT)
