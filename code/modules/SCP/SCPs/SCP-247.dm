...
/mob/living/simple_animal/hostile/scp247/proc/apply_memetic_effect(mob/living/carbon/human/viewer)
	...
	to_chat(viewer, "<span class='notice'>You see the most adorable little cat! It looks so soft and friendly...</span>")
	to_chat(viewer, "<span class='notice'>You feel an overwhelming urge to go pet it!</span>")
...
/mob/living/simple_animal/hostile/scp247/proc/draw_closer(mob/living/carbon/human/target)
	...
	if(prob(50))
		...
		to_chat(target, "<span class='notice'>You take a step toward the cute kitty...</span>")

	if(get_dist(src, target) <= 1)
		to_chat(target, "<span class='notice'>You reach out to pet the adorable cat...</span>")
...
/mob/living/simple_animal/hostile/scp247/proc/maul_target(mob/living/carbon/human/target)
	...
	if(get_dist(src, target) <= 1)
		visible_message("<span class='danger'>The cute cat suddenly reveals rows of enormous fangs and viciously mauls [target]!</span>")
...
/mob/living/simple_animal/hostile/scp247/AttackingTarget()
	...
	if(ishuman(target))
		...
		visible_message("<span class='danger'>The 'cat' lunges with terrifying force, revealing enormous claws and fangs!</span>")
...
/mob/living/simple_animal/hostile/scp247/examine(mob/user)
	...
	if(ishuman(user))
		...
		if(H.SCP)
			to_chat(user, "<span class='warning'>This is SCP-247, a Bengal tiger with a memetic disguise. It appears as a harmless house cat to viewers.</span>")
			to_chat(user, "<span class='danger'>Do NOT approach it. It is a dangerous predator.</span>")
		else
