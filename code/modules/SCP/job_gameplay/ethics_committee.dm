		...
		if(M)
			switch(severity)
				if(ETHICS_VIOLATION_MINOR)
					to_chat(M, "<span class='warning'>The Ethics Committee has issued a formal reprimand against you for: [violation_type].</span>")
				if(ETHICS_VIOLATION_MODERATE)
					to_chat(M, "<span class='warning'>The Ethics Committee has issued a formal warning against you for: [violation_type]. Further violations will result in disciplinary action.</span>")
				if(ETHICS_VIOLATION_SEVERE)
					to_chat(M, "<span class='danger'>The Ethics Committee has found you in severe violation of ethical protocols for: [violation_type]. Disciplinary action has been recommended.</span>")
				if(ETHICS_VIOLATION_CRITICAL)
					to_chat(M, "<span class='userdanger'>The Ethics Committee has found you in CRITICAL violation for: [violation_type]. Immediate suspension and tribunal referral recommended.</span>")
