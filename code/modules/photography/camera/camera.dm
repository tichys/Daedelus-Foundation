...
/obj/item/camera/proc/adjust_zoom(mob/user)
	...
	if(!desired_x || QDELETED(user) || QDELETED(src) || !user.canUseTopic(src, USE_CLOSE|USE_IGNORE_TK) || loc != user)
		return FALSE
	...
	if(!desired_y|| QDELETED(user) || QDELETED(src) || !user.canUseTopic(src, USE_CLOSE|USE_IGNORE_TK) || loc != user)
		return FALSE
