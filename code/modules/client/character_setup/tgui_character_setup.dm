/datum/character_setup_ui
	...
	New(mob/user)
		...
		if(user?.client?.prefs)
			...
			// Initialize faction/class state with a migration token if unset
	...
	ui_interact(mob/user, datum/tgui/ui)
		if (!prefs)
			if (user?.client?.prefs)
				...
				return FALSE
		// Ensure preview exists and is registered for this client before first open
		...
		if(!ui)
			...
			ui.open()
			// Force a preview update shortly after open
	...
	ui_static_data(mob/user)
		...
		// Species choices for customization
		...
		// Gender choices (defaults)
		data["gender_choices"] = list("male", "female", "plural")
		return data

	// Helper: list of SCP antag roles for filtering
	proc/get_scp_roles()
		return list(
	...
	ui_data(mob/user)
		var/list/data = list()
		// Ensure static datasets are available on dynamic payload for the frontend
		if (!faction_to_classes)
			faction_to_classes = get_faction_to_classes()
		if (!faction_lore)
			faction_lore = get_faction_lore()
		...
		// Species data for selection and id
		var/list/species_choices = list()
		var/list/species_names = list()
		for (var/species_id in get_selectable_species())
			var/datum/species/S = GLOB.species_list[species_id]
		...
		data["species_choices"] = species_choices
		data["species_names"] = species_names
		// Species id for selection state
		...
		// Faction/Class basic state (may be unset)
		...
		// Available jobs for current selection
		...
		// Antagonist preferences (SCP-only view)
		...
		// Languages: current selections map and catalog of available languages (path -> name)
		...
		// Loadout catalog summary (serialize entries)
		...
		// Appearance mods and Augments summaries
	...
	ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
		...
		switch(action)
			if ("loadout_toggle")
				// Defer to existing preference button_act interface
			...
			if ("appearance_mods_act")
				...
				if(params["add"])
					// Handle add mod action - use the existing preference system
					return mod_pref.button_act(usr, prefs, params)
				return mod_pref.button_act(usr, prefs, params)
			if ("augments_act")
				// Use the existing augments preference user_edit method
			...
			if ("open_preferences")
				// Open classic preferences UI as a bridge while we integrate customization pages
				return prefs.ui_interact(usr)
			if ("open_loadout")
				// Create independent loadout window
			...
			if ("set_job_priority")
				var/job = params["job"]
				var/level = text2num(params["level"]) // 1 low, 2 med, 3 high, null/off
				if (!job)
					return FALSE
				var/list/job_prefs = prefs.read_preference(/datum/preference/blob/job_priority) || list()
	...
	// Sync basic personnel record info on commit or high-priority job select
	proc/SYNC_PERSONNEL_RECORD(var/job_title)
